...
class BaseModel
{
    ...
    public function loadRequest($method, $arguments)
    {
        [$query, $replace] = call_user_func([$this, $method], $arguments);
        return $this->execute($query, $replace);
    }

    public function execute($query, $replace = [])
    {
        if(strrpos($query, 'SELECT') === false) return;
